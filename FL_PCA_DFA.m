...
% for    feature_length = (0.7:0.07:6.3)  
    ...
    for I_index = 1:size(folder_names,1)
        ...
        for Sensor_No = 1:5
            ...
            load(['D:\Liam Passport\Study 1 Data\processing_code\training_data_bases\TDB_',Individual_No,'_Feature_length_',num2str(round(100*feature_length)),'.mat'])
            % Centre the data set:
            ...
            load(['D:\Liam Passport\Study 1 Data\processing_code\training_data_bases\high_density\high_density_TDB_',Individual_No,'_Feature_length_',num2str(round(100*feature_length)),'.mat'])
            eval(['TDB_',num2str(Sensor_No),' = TDB_',num2str(Sensor_No),''';'])
              % Normalise the training data base:
            eval(['extent_for_loop = size(TDB_',num2str(Sensor_No),',1)'])
            eval(['extent_for_loop = size(TDB_',num2str(Sensor_No),',1)'])
...
%             criterion(str2num(Individual_No),Sensor_No,tr_counter) = (scatter_01*scatter_02/dist_1_2)*(scatter_01*scatter_03/dist_1_3)*(scatter_02*scatter_03/dist_2_3);
            ...
            title([' Individual = ', num2str(Individual_No) ' Sensor = ',num2str(Sensor_No),' Feat leng = ',num2str(feature_length),' criterion = ',num2str(criterion(str2num(Individual_No),Sensor_No,tr_counter))])   
            print(['D:\Liam Passport\Study 1 Data\processing_code\effect_of_feature_length\Scatter plots new criterion\Ind_',num2str(Individual_No),'_Sensor_',num2str(Sensor_No),'_FL_',num2str(round(100*feature_length)),'.tif'],'-dtiff','-r300')
            pause(0.1)
        end
    end
...
save entire_environment_updated_criterion_FL.mat