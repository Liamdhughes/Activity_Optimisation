...
tr_counter = 1;
  ...
  for temporal_resolution = 0.02:0.01:0.3  
%   for temporal_resolution = 0.3  
% for temporal_resolution = 0.3  
...
%  for Sensor_No = 4
            ...
            load(['D:\Liam Passport\Study 1 Data\processing_code\training_data_bases\TDB_',Individual_No,'_TR_',num2str(round(1000*temporal_resolution)),'.mat'])
            % Centre the data set:
...
%             pause(100)
            ...
            load(['D:\Liam Passport\Study 1 Data\processing_code\training_data_bases\high_density\high_density_TDB_',Individual_No,'_TR_',num2str(round(1000*temporal_resolution)),'.mat'])
            eval(['TDB_',num2str(Sensor_No),' = TDB_',num2str(Sensor_No),''';'])
            eval(['extent_for_loop = size(TDB_',num2str(Sensor_No),',1)'])
            for ms_No = 1:extent_for_loop
                eval(['TDB_',num2str(Sensor_No),'(ms_No,:) = (1/mean(TDB_',num2str(Sensor_No),'(ms_No,:)))*TDB_',num2str(Sensor_No),'(ms_No,:);'])
...
%             criterion(str2num(Individual_No),Sensor_No,tr_counter) = (scatter_01*scatter_02/dist_1_2)*(scatter_01*scatter_03/dist_1_3)*(scatter_02*scatter_03/dist_2_3);
            ...
            title([' Individual = ', num2str(Individual_No) 'Sensor = ',num2str(Sensor_No),'Temp Res = ',num2str(temporal_resolution),' criterion = ',num2str(criterion(str2num(Individual_No),Sensor_No,tr_counter))])   
            print(['D:\Liam Passport\Study 1 Data\processing_code\effect_of_temporal_resolution\Scatter plots new criterion\Ind_',num2str(Individual_No),'_Sensor_',num2str(Sensor_No),'_TR_',num2str(temporal_resolution),'.tif'],'-dtiff','-r300')
            pause(0.1)
...
save entire_environment_updated_criterion_TR.mat
